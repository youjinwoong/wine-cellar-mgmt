        ...
        {(wine.vivinoPrice || wine.wineSearcherPrice) && (() => {
          ...
          return (
            <div style={{ background: T.surface, border: `1px solid ${T.gold}44`, borderRadius: 8, padding: '10px 14px', marginBottom: 16 }}>
              <div style={{ fontSize: '0.66rem', color: T.gold, textTransform: 'uppercase', letterSpacing: '0.08em', marginBottom: 8, fontWeight: 600 }}>🌐 글로벌 시장가 (750ml)</div>
              <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                {vp && <div><div style={{ fontSize: '0.68rem', color: T.muted }}>Vivino</div><div style={{ fontWeight: 600, color: T.cream }}>${vp}{wine.vivinoRating && <span style={{ fontSize: '0.72rem', color: T.muted, marginLeft: 6 }}>⭐{wine.vivinoRating}</span>}</div></div>}
                {wp && <div><div style={{ fontSize: '0.68rem', color: T.muted }}>Wine-Searcher</div><div style={{ fontWeight: 600, color: T.cream }}>${wp}</div></div>}
                {vp && wp && <div><div style={{ fontSize: '0.68rem', color: T.gold, fontWeight: 600 }}>평균</div><div style={{ fontSize: '1.1rem', fontWeight: 700, color: T.gold }}>${avg}</div></div>}
              </div>
            </div>
          )
        })()}
