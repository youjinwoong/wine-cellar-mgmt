  ...
  async function runAI() {
    ...
    try {
      ...
      const data = await callAI([{
        role: 'user',
        content: `?Ä??"${q}"Î•?Vivino¬∑Wine-Searcher?êÏÑú Í≤Ä?âÌïòÍ≥?JSONÎß?Î∞òÌôò (ÎßàÌÅ¨?§Ïö¥ ?ÜÏù¥):
{"producer":"?ùÏÇ∞??,"region":"ÏßÄ??,"country":"Íµ??","grape":"?àÏ¢Ö","description":"?úÍµ≠??2Î¨∏Ïû•","imageUrl":"?¥Î?ÏßÄURL?êÎäîÎπàÎ¨∏?êÏó¥","vivinoPrice":Í∞ÄÍ≤©Ïà´?êÎòê?înull,"vivinoRating":?âÏ†ê?´Ïûê?êÎäînull,"wineSearcherPrice":750ml1Î≥ëÍ?Í≤©Ïà´?êÎòê?înull}`,
      ...
      console.error(e)
      if (e.message === 'API ???ÜÏùå') alert('?ôÔ∏è ?§Ï†ï?êÏÑú Claude API ?§Î? ?ÖÎ†•?¥Ï£º?∏Ïöî')
      setImgErr(true)
    }
  ...
  function submit() {
    if (!form.name.trim()) { alert('?Ä???¥Î¶Ñ???ÖÎ†•?òÏÑ∏??); return }
    onAdd({
      ...form, ...(aiInfo || {}),
      id: uid(),
    ...
    <div className="modal-overlay" onClick={e => e.target === e.currentTarget && onClose()}>
      <div className="modal-box">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <h2 style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.4rem', color: T.cream }}>?Ä??Ï∂îÍ?</h2>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: T.muted, fontSize: '1.2rem' }}>??/button>
        </div>

        {/* Name + AI */}
        <div style={{ marginBottom: 12 }}>
          <label style={lbl}>?Ä???¥Î¶Ñ *</label>
          <div style={{ display: 'flex', gap: 8 }}>
            <input value={form.name} onChange={e => set('name', e.target.value)}
              placeholder="?? Ch√¢teau Margaux" style={{ flex: 1 }}
        ...
        {aiInfo && (
          <div style={{ background: T.surface, border: `1px solid ${T.gold}44`, borderRadius: 8, padding: '12px 14px', marginBottom: 12 }}>
            <div style={{ color: T.gold, fontWeight: 600, marginBottom: 8, fontSize: '0.8rem' }}>??AI ?ïÎ≥¥</div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '4px 16px', marginBottom: 8 }}>
              {[['?ùÏÇ∞??, aiInfo.producer], ['ÏßÄ??, aiInfo.region], ['Íµ??', aiInfo.country], ['?àÏ¢Ö', aiInfo.grape]].map(([k, v]) =>
            ...
            {(aiInfo.vivinoPrice || aiInfo.wineSearcherPrice) && (
              <div style={{ borderTop: `1px solid ${T.border}`, paddingTop: 8, display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
                <span style={{ fontSize: '0.72rem', color: T.muted, textTransform: 'uppercase' }}>?úÏû•Í∞Ä</span>
                ...
                {aiInfo.vivinoPrice && aiInfo.wineSearcherPrice && <span style={{ fontSize: '0.78rem', color: T.gold, fontWeight: 600 }}>?âÍ∑† ${Math.round((aiInfo.vivinoPrice + aiInfo.wineSearcherPrice) / 2)}</span>}
              </div>
            )}
        ...
        <div style={G}>
          <div><label style={lbl}>ÎπàÌã∞ÏßÄ</label><input value={form.vintage} onChange={e => set('vintage', e.target.value)} type="number" placeholder="?? 2018" /></div>
          <div><label style={lbl}>?òÎüâ (Î≥?</label><input value={form.qty} onChange={e => set('qty', e.target.value)} type="number" min="1" /></div>
        </div>
        <div style={G}>
          <div><label style={lbl}>Íµ¨Îß§??/label><input value={form.purchaseDate} onChange={e => set('purchaseDate', e.target.value)} type="date" /></div>
          <div><label style={lbl}>Íµ¨Îß§Í∞ÄÍ≤?(??</label><input value={form.price} onChange={e => set('price', e.target.value)} type="number" placeholder="?? 150000" /></div>
        </div>
        <div style={G}>
          <div>
            <label style={lbl}>?Ä??/label>
          ...
          <div>
            <label style={lbl}>Ïπ?Î≤àÌò∏</label>
            <select value={form.slot} onChange={e => set('slot', e.target.value)}>
              {getSlots(curCellar).map(s => <option key={s} value={s}>{s}Î≤?Ïπ?/option>)}
            </select>
          </div>
        </div>
        ...
        <div style={{ marginBottom: 22 }}>
          <label style={lbl}>Î©îÎ™®</label>
          <textarea value={form.notes} onChange={e => set('notes', e.target.value)} rows={2} placeholder="?åÏù¥?§ÌåÖ ?∏Ìä∏, Î≥¥Í? Î©îÎ™® ??.." style={{ resize: 'vertical' }} />
        </div>
        <div style={{ display: 'flex', gap: 10, justifyContent: 'flex-end' }}>
          <Btn variant="ghost" onClick={onClose}>Ï∑®ÏÜå</Btn>
          <Btn variant="gold" onClick={submit}>?Ä??/Btn>
        </div>
      </div>
    </div>
  )
