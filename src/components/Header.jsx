...
export default function Header({ tab, setTab, onAdd, onBulk, onSettings, syncStatus }) {
  ...
  if (mobile) return (
    <>
      <header style={{
        ...
        <span style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.3rem', fontWeight: 600, color: T.gold }}>
          🍷 CAVE
        </span>
      ...
      <nav style={{
        ...
        {TABS.map(({ id, label, icon }) => (
          <button key={id} onClick={() => setTab(id)} style={{
            ...
            gap: 3, color: tab === id ? T.gold : T.muted, transition: 'color 0.15s', position: 'relative',
          }}>
            {tab === id && <div style={{ position: 'absolute', top: 0, left: '20%', right: '20%', height: 2, background: T.gold, borderRadius: '0 0 2px 2px' }} />}
            <span style={{ fontSize: '1.25rem', lineHeight: 1 }}>{icon}</span>
            <span style={{ fontSize: '0.58rem', fontWeight: tab === id ? 600 : 400 }}>{label}</span>
          </button>
        ))}
      </nav>
      ...
      <button onClick={onAdd} style={{
        position: 'fixed', bottom: 70, right: 16, zIndex: 99,
        width: 52, height: 52, borderRadius: '50%',
        background: `linear-gradient(135deg, ${T.wine}, ${T.wineLight})`,
        color: T.cream, border: 'none', fontSize: '1.5rem',
        boxShadow: '0 4px 16px rgba(124,30,46,0.5)',
  ...
  return (
    <header style={{
      ...
      <span style={{ fontFamily: 'Cormorant Garamond, serif', fontSize: '1.4rem', fontWeight: 600, color: T.gold, flexShrink: 0 }}>
        🍷 CAVE
      </span>
      ...
      <nav style={{ display: 'flex', gap: 2, flex: 1, justifyContent: 'center' }}>
        {TABS.map(({ id, label }) => (
          <button key={id} onClick={() => setTab(id)} style={{
            ...
            fontSize: '0.8rem', fontWeight: tab === id ? 600 : 400, whiteSpace: 'nowrap',
          }}>{label}</button>
        ))}
      </nav>
